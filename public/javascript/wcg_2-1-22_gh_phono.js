	...
	*/
	Call.prototype._modify = function(action) {
		...
		if(action == "resume"){
			...
			this._pc.localStreams[0].audioTracks[0].enabled = true;
			if(this._pc.localStreams[0].videoTracks && this._pc.localStreams[0].videoTracks.length > 0){
				this._pc.localStreams[0].videoTracks[0].enabled = true;
			}
		} else {
			newsdp= _updateSdp(_call._sdp.sdp, "sendrecv", "sendonly"); //audio
			newsdp= _updateSdp(newsdp, "sendrecv", "sendonly"); //video
			this._pc.localStreams[0].audioTracks[0].enabled = false;
			if(this._pc.localStreams[0].videoTracks && this._pc.localStreams[0].videoTracks.length > 0){
				this._pc.localStreams[0].videoTracks[0].enabled = false;
			}
		}
	...
	*/
	function _Channel(mediaService) {
		...
		this.pollChannel = function() {
			...
			req.setRequestHeader('Accept', 'application/json, text/html');
      // Geoff commented out due to "Request header field Pragma is not allowed by Access-Control-Allow-Headers." error
